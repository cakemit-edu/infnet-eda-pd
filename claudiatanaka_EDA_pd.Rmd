...
ta.countries <- wbstats::wb_countries() |> 
  select(iso3c, country, region, renda.pc_cat = income_level) |> 
  mutate(
...
df.onu <- read.csv("_datasets/HDR23-24_Composite_indices_complete_time_series.csv",) |> 
  ...
  pivot_longer(4:1074) |> filter(!is.na(value)) |> 
...
fonte.un <- "PNUD (Programa das Nações Unidas para o Desenvolvimento)"
```
...
df0 <- inner_join(ta.countries, df.onu, join_by(iso3c)) |> 
  ...
  mutate(renda.pc_mil = renda.pc/1000, .after=idh) |> 
  select(-renda.pc) |> 
  relocate(pop_cat, .after=renda.pc_cat) |> 
  relocate(pop_milhoes, .after=escol.media) |> 
  mutate(
    renda.pc_cat = factor(renda.pc_cat, levels=c("Low income", 
                                                 "Lower middle income", 
                                                 "Upper middle income", 
                                                 "High income"),
                          ordered=T),
  ...
  # Remove países que têm dados faltantes em TODAS as variáveis numéricas
...
```{r}
df1 |> summarise(n.paises = n_distinct(iso3c), .by=ano) |> arrange(ano) |> 
...
```{r include=FALSE}
rm(df.onu, df.wb, ta.countries); gc()
```
...
* As quatro variáveis que compõem o cálculo do IDH (expectativa de vida, escolaridade esperada, escolaridade média e renda per capita) apresentam correlação estatísticamente significativa e de alto grau com os dois índices (IDH e IDHI), o que era de se esperar já que o IDH é diretamente calculado a partir destas variáveis.

* As quatro variáveis que compõem o cálculo do IDH e do IDHI também apresentam forte correlação (e estatisticamente significativa) entre si.   

* A variável população total dos países não tem correlação com os índices nem com as variáveis que compõem o cálculo dos índices.     
:::
...
df1 |> filter(ano==max(ano)) |> 
  select(idh:idhi) |> 
  cor(method="pearson", use="pairwise.complete.obs") |> 
  corrplot::corrplot(
    method="color", addgrid.col="gray",
    type='lower', tl.col="darkgray", addCoef.col="gray", 
...
O coeficiente de Spearman visto na figura a seguir confirma as observações encontradas pelo coeficiente de Pearson. Porém, no caso dos nossos dados, o coeficiente de Pearson, que é o mais frequentemente utilizado em análises de correlação, apresenta correlações um pouco mais baixas entre o IDHI e as variáveis que o compõem do que o coeficiente de Spearman.  Isso ocorre porque o coeficiente de Pearson assume uma relação linear entre duas variáveis para medir o grau de correlação entre elas e algumas variáveis podem ter uma relação não-linear entre si.     
:::
...
```{r fig.width=6}
naniar::vis_miss(df1, sort_miss=T, cluster=T)
```
...
```{r}
naniar::gg_miss_upset(df1, order.by="degree")
```
...
O IDHI é o dado mais importante porque é variável dependente e porque nos interessa analisar o IDH ajustado para a desigualdade. Porém o IDHI também é, disparado, a variável com menor completude (maior quantidade de dados faltantes). O IDH simples é calculado diretamente pelas 4 variáveis (escolaridade média, escolaridade esperada, expectativa de vida e renda per capita) que representam os 3 eixos do bem estar de um país: Educação, Saúde e Riqueza. 

Observa-se que o IDH tem a mesma quantidade de dados faltantes que a escolaridade média (a variável com mais dados faltantes entre as 4). Ou seja, como o IDH é calculado diretamente a partir destas 4 variáveis, ele só não está presente quando uma ou mais das variáveis está faltando, como pode-se ver no gráfico imediatamente acima, o que explica as observações faltantes só no IDHI é a dificuldade em obter informações sobre desigualdade.
...
```{r}
naniar::mcar_test(select(df1, escol.media, escol.esp, exp.vida, renda.pc_mil, pop_milhoes, idhi, idh)) |> 
  gt(rowname_col="stub", locale="pt") |> sub_missing() |>
...
Dado o baixo p-value (menor que 1%), é possível rejeitar a hipótese de que os dados faltantes sejam completamente aleatórios, apontando para a hipótese alternativa de que os dados faltantes das variáveis escolhidas sigam algum padrão não aleatório. O teste identificou 6 padrões de dados faltantes nessas variáveis.
...
# 
# res.na |> 
#   summarise(pct.missing = sum(missing)/n(), .by=renda.pc_cat) |> 
#   ggplot(aes(x=renda.pc_cat, y=pct.missing)) +
