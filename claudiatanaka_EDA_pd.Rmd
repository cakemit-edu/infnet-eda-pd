...
No caso da população, que tem uma curtose elevada, é possível que os dois países que são outliers extremos (China e Índia têm população cerca de 4x a dos Estados Unidos, que é o terceiro maior) estejam nos impedindo de enxergar a distribuição.\
...
```{r include=FALSE}
rm(p1,p2,p3, df1.norm, df1.padr); gc()
```
...
Se as duas distribuições comparadas forem semelhantes, os pontos no Q–Q plot ficarão aproximadamente na linha de identidade $y = x$. Se as distribuições estiverem linearmente relacionadas, os pontos no Q–Q plot ficarão aproximadamente em uma linha reta, mas não necessariamente na linha $y = x$. 

Assim, um Q–Q plot é frequentemente usado para comparar as formas das distribuições, fornecendo uma visão gráfica de como propriedades como localização, escala e assimetria são semelhantes ou diferentes nas duas distribuições. 

Os Q–Q plots também podem ser usados para comparar coleções de dados ou distribuições teóricas ou para comparar um conjunto de dados com um modelo teórico, podendo fornecer uma avaliação visual da qualidade do ajuste, em vez de ser reduzida a uma estatística de resumo numérico. 
...
suppressWarnings(print(
  ggpubr::ggqqplot(
    df1 |> 
      mutate(
        ano = as.factor(ano),    
        across(idh:idhi, scale) # padroniza a escala das variáveis numéricas (z-score)
      ), 
    x = c("idh", "idhi",  
...
O teste de Shapiro-Wilk (Shapiro e Wilk, 1965) testa a hipótese nula de que uma amostra pertence a uma população com distribuição normal, usando a estatística $W$. Dessa forma, se o p-valor for menor que um limite pré-determinado $\alpha$ então podemos rejeitar a hipótese de que a amostra foi coletada de uma população com distribuição normal, o que aponta para a hipótese alternativa de que a amostra não pertence a uma distribuição normal. 
...
A única variável da amostra que dá indícios de se aproximar de uma distribuição normal é a escolaridade esperada - tanto pela visualização gráfica dos histogramas quanto dos Q-Q plots quanto pelos resultados do teste de Shapiro-Wilk nos dois anos.
...
A variável população (que não entra no cálculo do IDH/IDHI) apresenta tendência a outliers extremos e confirma o que havia sido observado com uma curtose bastante alta nos dois anos (P03).

Na comparação de 2012 com 2022 dos histogramas, os países aparentam ter aumentado os anos de escolaridade média e de escolaridade esperada.

No caso da expectativa de vida, é difícil determinar se a distribuição dos países melhorou ou piorou nessa década pois apesar de haver alguma queda na densidade dos bins inferiores, há também uma queda expressiva no penúltimo bin (~75-80 anos).

No caso da renda per capita, a distribuiçao parece ter piorado, com uma concentração maior de países no primeiro bin. Considerando que os valores estão expressos em US$ constantes (ajustados para a inflação do período), os poucos países mais ricos tiveram ganhos reais no valor da renda per capita média e se tornaram mais ricos, aumentando a distância em relação à renda média dos países mais pobres.
:::
...
df0.mice <- df0 |> select(-country) |> 
  mutate(across(c(iso3c, region), as.factor))
...
O código a seguir faz imputação múltipla para preencher os dados faltantes pela metodologia `pmm` (predictive mean match) no pacote `mice` (van Buuren, 2011) sobre a base com a série histórica completa. 


```{r}
res.pmm <- mice::mice(
  df0.mice, method="pmm", printFlag=F, seed=500,
  m=m.imp,  # número de partições
...
A seguir avalia-se o resultado das imputações na variável IDHI e compara-se as distribuições das variáveis antes e depois do preenchimento:

\

**Scatterplot de IDHI x IDH**

```{r fig.width=6}
xyplot(res.pmm, idhi ~ idh, col = c("grey", mdc(2)))
```


::: {.alert .alert-block .alert-info}
Seria esperado observar que o padrão dos pontos na cor magenta (preenchidos) se aproximasse ou ficasse sobreposto ao padrão dos pontos cinza (originais). Esse gráfico mostra que o padrão das imputações e o padrão dos dados originais são completamente diferentes pelo método pmm.
:::

\

**Densidades das imputações x dados originais**

```{r fig.width=6}
densityplot(res.pmm, ~ idhi, col = c("grey", mdc(2)))
```


::: {.alert .alert-block .alert-info}
A densidade dos dados preenchidos para cada imputação aparece em magenta e a densidade dos dados originais aparece em cinza. Esperaría-se que a densidade dos dados imputados fosse similar à dos dados originais, o que não ocorre.
:::
...
Comparação da distribuição de variáveis selecionadas (numéricas e categóricas). 
Pontos em magenta ressaltam as observações com dados de IDHI faltantes no dataset completo.

```{r fig.asp=1, fig.width=8}
mice::stripplot(
  res.pmm, idhi+exp.vidai+escoli+rendai+idh+rendapc_cat+pop_cat+idh_cat+region ~ .imp,
  na = idhi, 
...
::: {.alert .alert-block .alert-info}
No gráfico do IDHI, nota-se que o método pmm concentrou os preenchimentos nos limites superior ou inferior dos dados originais. Porém, observa-se nas outras variáveis que as observações sem IDHI estão bem distribuídas/ espalhadas nas outras variáveis.
:::
...
O código a seguir faz imputação múltipla pela metodologia `midastouch` (pmm ponderado) no pacote `mice` sobre a base com a série histórica completa.


```{r}
res.midas <- mice::mice(
  df0.mice, method="midastouch", printFlag=F, seed=1,
  m=m.imp,  # número de partições
  maxit=5,  # número de iterações
)
```
...
```{r fig.width=6}
xyplot(res.midas, idhi ~ idh, col = c("grey", mdc(2)))
```


::: {.alert .alert-block .alert-info}
Usando a metodologia do "toque de midas" os padrões ficam bem mais próximos, porém ainda há alguns vazios nas áreas cinza.
:::

\

**Sobreposição das densidades das imputações x dados originais**

```{r fig.width=6}
densityplot(res.midas, ~ idhi, col = c("grey", mdc(2)))
```


::: {.alert .alert-block .alert-info}
Apesar da melhoria notável na densidade comparado ao método pmm, esperaría-se que a densidade dos dados imputados fosse mais similar à dos dados originais.
:::
...
```{r fig.asp=1, fig.width=8}
mice::stripplot(
  res.midas, idhi+exp.vidai+escoli+rendai+idh+rendapc_cat+pop_cat+idh_cat+region ~ .imp,
  na = idhi, 
  col = c("grey", mdc(2)), pch = c(1, 20)
)
```
...
O método `midastouch` teve resultados melhores que o `pmm`, mas é "custoso" em termos de processamento e o resultado não me parece suficiente. Seria necessário procurar outros métodos, talvez em outros pacotes como `simputation`.
:::
...
[R-bloggers: Imputing missing data with R; MICE package](https://www.r-bloggers.com/2015/10/imputing-missing-data-with-r-mice-package/)
...
Freedman, D. and Diaconis, P. (1981). *On the histogram as a density estimator.* Zeitschrift fur Wahrscheinlichkeitstheorie und verwandte Gebiete, 57, 453–476. https://doi.org/10.1007/BF01025868

\

Little, Roderick J. A. 1988. *A Test of Missing Completely at Random for Multivariate Data with Missing Values.* Journal of the American Statistical Association, 83(404), 1198–1202. https://doi.org/10.2307/2290157
...
van Buuren, S., & Groothuis-Oudshoorn, K. (2011). [*mice: Multivariate Imputation by Chained Equations in R*](https://www.jstatsoft.org/article/view/v045i03). Journal of Statistical Software, 45(3), 1–67. https://doi.org/10.18637/jss.v045.i03
